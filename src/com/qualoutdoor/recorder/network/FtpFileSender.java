...
/**Child of Sender Class, implementing file sending with FTP protocol. An FTP connection is limited to a file
 * sending then only sendFile method will be implemented. Attributes of this class are the specific parameters
 * of a FTP connection*/
...
public class FtpFileSender implements Sender{
	...
	/**
	 * Implementation of sendFile method : a new connection is opened, file is transmitted then connection is closed
	 */
	@Override
	public boolean sendFile(String url,String fileName,InputStream content){
		...
		try {
			...
			//enabling connection to handle outputs. Now inputs can't be handle anymore
			connection.setDoOutput(true);
			//Getting the flow to write into connection
			OutputStream os = connection.getOutputStream();
			
			//Setting Reading/Writing mecanisme : reading from the given content and writing into the new output stream	
			//temporary buffer
        	...
        	response = true;
        	os.close();
        	//if an exception occurred, transmission is considered unsuccessful. 
