...
public class EmailFileSender {
	...
	public static void sendFileByEmail(Context context, String dest, FileToUpload file){
		...
		if (!rfc2822.matcher(dest).matches()) {
			...
			String stats;
			java.util.Scanner s = new java.util.Scanner(file.getContent()).useDelimiter("\\A");
	        if(s.hasNext()){
	        	stats = s.next();
	        }else{
	        	stats="";
