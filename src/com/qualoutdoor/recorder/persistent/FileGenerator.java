...
public class FileGenerator extends AsyncTask<Void, Void, ByteArrayOutputStream> {
	...
	private FileReadyListener callback;//l'objet sur lequel appliquer la methode de callbak une fois la tache termin�e
	//SOLUTION PROVISOIRE
	...
	/*public void nodeRetranscription(DataBaseTreeManager managerWriter){
		try{
			...
			if(managerWriter.getCursor().getLevel()==1){
				int refFeuille = managerWriter.getCursor().getReference();//on r�cupere la reference de la feuille
	...
	public void tablesRetransciption(DataBaseTreeManager managerWriter){
		try{
			while(managerWriter.moveToNextLine()){//on lit la nouvelle ligne
				...
				if(managerWriter.getCursor().getLevel()==7){//si la ligne correspond � une feuille
					ArrayList<String> details = connecteur.getLeafDetails(managerWriter.getCursor().getReference());//on demande ses d�tails au connecteur
					this.file.write((managerWriter.getCursor().getLevel()+"/").getBytes());//ouverture d'un noeud dans le fichier correspondant � la feuille
					int compteurslash1 = 1;
					for(String field : details){//on inscrit � la suite ses d�tails
						this.file.write(field.getBytes());
						if(compteurslash1!=details.size()){
							this.file.write("/".getBytes());//pour chaque �l�ment non dernier de la liste on les fait suivre d'un slash
						}
						compteurslash1++;
					}
					this.file.write(";".getBytes());
					...
					this.file.write((levelNode+"/"+refNode+"$").getBytes());//ouverture d'un noeud + �criture de sa reference dans le fichier
				}
			}
		}catch(DataBaseException e){
			e.printStackTrace();
